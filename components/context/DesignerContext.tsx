...
export type DesignerContextType = {
  ...
  leftView: SidebarViewType
  setLeftView: Dispatch<SetStateAction<SidebarViewType>>
...
export default function DesignerContextProvider({
  ...
  const updateSelectedParents = (element: FormElementInstance) => {
    const newBroadCumb = [
      element,
      ...findParentsRecursiveWithLevel(element, elements, 0),
    ]
  ...
  function duplicateItems(
    originalArray: FormElementInstance[],
    targetPage: PageType,
    newPage: PageType
  ): FormElementInstance[] {
    const duplicateMap: Record<string, string> = {}
    const duplicatedArray: FormElementInstance[] = []
    ;[...originalArray]
      .filter((item: FormElementInstance) => item.page == targetPage.id)
      .forEach((item: FormElementInstance) => {
        const duplicatedItem = FormElements[item.type].construct(
          ulid(10),
          item.index,
          item.parentId,
          newPage.id,
  ...
  function duplicatePage(page: PageType, index: number) {
    ...
    const getAllStatsWith: Array<PageType> = []
    pages.forEach((page: PageType) => {
      if (
        Number(
          String(page.index).startsWith(String(Math.floor(pages[index].index)))
        )
      ) {
    ...
    const newIndex = Number(
      (Math.max(...getAllStatsWith.map((page) => page.index)) + 0.001).toFixed(
        3
      )
    )
    const newPage: PageType = {
      id: ulid(),
      index: newIndex,
      name: `Duplicate-${newIndex}`,
    }
    const getAllItemsInPage: FormElementInstance[] = duplicateItems(
      cloneELements,
      page,
      newPage
    )
    clonePages.splice(index + 1, 0, newPage)
    if (!getAllItemsInPage) {
      return
    }
    setPages([...clonePages])
    setElements((prev: Array<FormElementInstance>) => [
      ...prev,
      ...getAllItemsInPage,
    ])
  ...
  function newPage(page: PageType, index: number) {
    ...
    const pageNumber =
      Math.floor(Math.max(...clonePages.map((page) => page.index))) + 1
    const newPage = {
      id: ulid(10),
      index: pageNumber,
  ...
  function deletePage(page: PageType, index: number) {
    ...
    setElements((prev: FormElementInstance[]) =>
      prev.filter((item: FormElementInstance) => item.page != page.id)
    )
  ...
  function deleteElement(element: FormElementInstance) {
    setElements((prev) => {
      ...
      const allchildsId: Array<string> = getChildNodes(
        element,
        cloneElements
      ).map((child) => child.id)
      const itemsToDelete = [element.id, ...allchildsId]
      const updatedElements = cloneElements.filter(
  ...
  return (
    <DesignerContext.Provider value={{ ...context, context: context }}>
      {children}
