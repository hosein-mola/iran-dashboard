...
function DesignerComponent({ elementInstance }: { elementInstance: FormElementInstance }) {
  ...
  >
    {elements.filter(el => el.parentId == element.id).sort((a: FormElementInstance, b: FormElementInstance) => a.index - b.index).map((_element, index) => {
      return <DesignerElementWrapper key={element.id} element={_element} index={index} row={element.extraAttributes['flexDirection'] == 'row' ? true : false} />
    })}
  </div >
}
...
function FormComponent({
  ...
  >
    {elements.filter(el => el.parentId == element.id).sort((a: FormElementInstance, b: FormElementInstance) => a.index - b.index).map((element: FormElementInstance, index) => {
      const FormElementInstance: any = FormElements[element.type].formComponent;
        ...
        <AccordionItem value="item-2" className='flex flex-col gap-2'>
          ...
          <AccordionContent className=''>
            <div className='mt-2 flex flex-row gap-1 p-1'>
              <FormField control={form.control} name={'flex'} render={({ field }) => {
                return <FormItem>
