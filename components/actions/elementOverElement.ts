...
function reindexItems(items: any, dragIndex: number, dropIndex: number, isAbove: boolean, selectedPage: string): any {
...
export function elementOverElement(event: DragEndEvent, selectedPage: PageType, context: ContextType) {
    const { elements, addElement, setElements, setSelectedElement, updateIndex, removeElement } = context;
    ...
    if (draggingDesignerElementOverAnotherDesignerElement) {
        ...
        let newIndexForActiveElement = reindexItems(elements, activeElementIndex, overElementIndex, isDroppingOverDesignerElementTopHalf, selectedPage.id);
