  ...
  return (
    <Tabs defaultValue={theme} className="w-full">
      <TabsList className="border w-full">
        <TabsTrigger className={"cursor-pointer"} value={"light"} onClick={() => setTheme("light")}>
          <SunIcon className="h-[1.2rem] w-[1.2rem] text-yellow-600" />
        </TabsTrigger>
        <TabsTrigger className={"cursor-pointer"} value={"dark"} onClick={() => setTheme("dark")}>
          <MoonIcon className="h-[1.2rem] w-[1.2rem] text-blue-700" />
        </TabsTrigger>
        <TabsTrigger className={"cursor-pointer"} value={"system"} onClick={() => setTheme("system")}>
