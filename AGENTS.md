...
- `app/` holds the Next.js route handlers and server components for the dashboard experience, with nested folders mirroring the pages.
- `components/` is the shared UI library; keep view logic, hooks, and helpers that multiple routes consume there.
- `api/` and `schemas/` contain edge functions and validation rules used by the backend surface, while `lib/` stores utility wrappers (e.g., data fetching, analytics).
- `public/` carries static assets; `package/` includes runtime patches that get copied into `node_modules` via `npm run apply-patches`.

## Build, Test, and Development Commands

- `npm run dev` — boots Next.js with Turbopack on port 3000 for local iteration.
- `npm run build` — compiles all Next.js routes and layouts; run before deployment or previewing production builds.
- `npm run start` — serves the compiled app after `npm run build`; only for production simulations.
- `npm run lint` — runs `next lint` with the provided ESLint/Prettier stack to catch code-style and typing issues.
- `npm run pm2` — launches the app via `pm2` using `pm2.json` for process management in staging/production.
- `npm run apply-patches` — copies the contents of `package/` into `node_modules/` for Windows/Linux compatibility patches.

## Coding Style & Naming Conventions

- JavaScript/TypeScript files follow Next.js conventions: PascalCase for React components and hooks, camelCase for functions/props, and `app/route.ts` style file names for API routes.
- Tailwind CSS utility classes are combined with `clsx`; prefer descriptive names when composing `cn()` wrappers instead of long inline class strings.
- The project uses ESLint 9 + `eslint-config-next` and Prettier 3 with `prettier-plugin-tailwindcss`; format files with `npm run lint` or `npm run lint -- --fix` before commits.
- Keep indentation to two spaces (the default for Next/Prettier in this project).

## Testing Guidelines

- No automated test suite is committed yet; rely on `npm run lint` to validate typing and formatting as a lightweight quality gate.
- If you add tests (e.g., Jest or Playwright), name them `*.test.tsx`/`*.spec.ts` and colocate them beside the component/module they cover.
- Document how to run new test commands near the scripts section and verify they pass before merging.

## Commit & Pull Request Guidelines

- Follow existing git log style: short, imperative summaries like “add pm2 config” or “migrate to Next 16”; include scope if helpful (e.g., `dashboard: refresh charts`).
- Each PR should explain what changed, why it matters, and how to test it; link the relevant issue or ticket, and attach screenshots/gifs for UI tweaks.
- Run `npm run lint` before committing; mention the command in the PR checklist along with any manual validation steps you took.

## Security & Configuration Tips

- Secrets/config reside outside the repo; inject them at runtime via environment variables defined in deployment tooling.
- Keep `package/` patches in sync with `node_modules` by running `npm run apply-patches` after pulling new dependencies or updating packages.
  .
  ├── actions
  │   └── form.ts
  ├── api
...
how to structure nextjs for each module
